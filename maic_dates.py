...
# files = date_to_maic('2005-01-01', '2022-01-01')
...
# # # # call file_compiler with compile_files
# compile_files(files)
#%%
...
test_date = date(2005, 1, 1)  # date up to which we compile
end_date = date(2022, 1, 1)  # final date compiled
delta = relativedelta(months=1)  # time increase between runs

# compile files between start date and test date, adding delta each time
while test_date <= end_date:
    compile_files(date_to_maic(start_date, test_date))
    test_date += delta
